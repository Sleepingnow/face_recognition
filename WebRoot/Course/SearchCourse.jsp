...
<%
	...
	try{
		...
		res = ps.executeQuery();
		if(!res.next())
			out.println("不存在姓名为" + Cname + "的课程，查找失败！");
		else{	
			out.print("<h2>查到课程情况如下</h2>");
			ps = con.prepareStatement("select * from Course where Cname = ?");
			ps.setString(1, Cname);	
			res = ps.executeQuery();
		...
		ps = con.prepareStatement("select * from CourseClass where Cname = ?");
		ps.setString(1, Cname);
		res = ps.executeQuery();
		if(!res.next()){
			int i;
		...
		ps = con.prepareStatement("select * from Attendence where Cname = ?");
		ps.setString(1, Cname);
		res = ps.executeQuery();
		if(!res.next())
			con.close();
		else{	
			out.print("<h2>所查课程当天签到情况</h2>");
			ps = con.prepareStatement("select * from Attendence where Cname = ?");
			ps.setString(1, Cname);	
			res = ps.executeQuery();
			out.print("<table class = tab id = tab3 border=2>");
			out.print("<tr>");
			out.print("<th width = 100>"+"课程号");
			out.print("<th width = 100>"+"课程名");
			out.print("<th width = 100>"+"学号");
			out.print("<th width = 100>"+"学生名");
			out.print("<th width = 100>"+"签到时间");
			out.print("</tr>");
			while(res.next()){
				out.print("<tr>");
					out.print("<td>"+res.getString(1)+"</td>");
