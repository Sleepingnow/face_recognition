...
<%
	...
	try{
		...
		ps = con.prepareStatement("select * from Teacher where Tname = ?");
		ps.setString(1, Tname);
		res = ps.executeQuery();
		if(!res.next())
			out.println("不存在姓名为" + Tname + "的教师，查找失败！");
		else{	
			out.print("<h2>查到教师情况如下</h2>");
			ps = con.prepareStatement("select * from Teacher where Tname = ?");
			ps.setString(1, Tname);	
			res = ps.executeQuery();
			out.print("<table class = tab id = tab1 border=2>");
			out.print("<tr>");
			out.print("<th width = 100>"+"教师工号");
			out.print("<th width = 100>"+"姓名");
			out.print("<th width = 100>"+"性别");
			out.print("<th width = 100>"+"研究方向");
			out.print("</tr>");
			while(res.next()){
				out.print("<tr>");
		...
		ps = con.prepareStatement("select * from Course where Tname = ?");
		ps.setString(1, Tname);
		res = ps.executeQuery();
		if(!res.next())
			con.close();
		else{	
			out.print("<h2>所查教师授课情况</h2>");
			ps = con.prepareStatement("select * from Course where Tname = ?");
			ps.setString(1, Tname);	
			res = ps.executeQuery();
